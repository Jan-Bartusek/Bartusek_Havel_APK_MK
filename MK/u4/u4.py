...
segments = []
current = []
with open("/Users/petis/Univerzita_Karlova/2_sem/Mat_kar/ukol_4/rusko.txt") as f:
    for line in f:
        line = line.strip()
        if line.startswith('#') or line == '':
            if current:
                segments.append(array(current))
                current = []
        else:
            parts = line.split()
            current.append([float(parts[1]), float(parts[0])])
if current:
    segments.append(array(current))

#Draw points
for seg in segments:
    Xc, Yc, ac, bc = project(proj_name, R, seg[:, 0], seg[:, 1], lat0, lon0)
